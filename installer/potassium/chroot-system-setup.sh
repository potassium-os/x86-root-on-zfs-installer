...
function _main () {
  ...
  sudo -E mkdir /mnt/etc/zfs
  sudo -E cp /etc/zfs/zpool.cache /mnt/etc/zfs/

  TARGET_HOSTNAME=$(yq '.hostname' <<< "${INSTALLER_CONF_YAML}" | sed 's/"//g')
  export TARGET_HOSTNAME

  sudo -E hostnamectl set-hostname "${TARGET_HOSTNAME}"
  hostname | sudo -E tee /mnt/etc/hostname > /dev/null

  cat <<EOF | sudo -E tee /mnt/etc/hosts > /dev/null
127.0.0.1 localhost
127.0.1.1 ${TARGET_HOSTNAME}
EOF

  NETWORK_INTERFACES=$(yq '.network_interfaces[]' <<< "${INSTALLER_CONF_YAML}" | sed 's/"//g')
  export NETWORK_INTERFACES

  SSH_PUBKEY=$(yq '.ssh_pubkey' <<< "${INSTALLER_CONF_YAML}" | sed 's/"//g')
  export SSH_PUBKEY

  KERNEL_CMDLINE=$(yq '.kernel_cmdline' <<< "${INSTALLER_CONF_YAML}" | sed 's/"//g')
  export KERNEL_CMDLINE

  ROOT_PASSWORD=$(yq '.root_password' <<< "${INSTALLER_CONF_YAML}" | sed 's/"//g')
  export ROOT_PASSWORD

  _log "${NETWORK_INTERFACES}"

  sudo -E mkdir -p /mnt/root/.ssh
  echo "${SSH_PUBKEY}" | sudo tee /mnt/root/.ssh/authorized_keys > /dev/null

  cat <<'EOF' | sudo tee /mnt/etc/network/interfaces > /dev/null
auto lo
...
EOF
  ...
  sudo mkdir -p /mnt/etc/preseed
  cat <<'EOF' | sudo tee /mnt/etc/preseed/console-setup.conf > /dev/null
console-setup	console-setup/fontsize-text47	select	8x18
console-setup	console-setup/codeset47	select	# Latin1 and Latin5 - western Europe and Turkic languages
console-setup	console-setup/fontsize-fb47	select	16x28 (framebuffer only)
console-setup	console-setup/fontface47	select	VGA
console-setup	console-setup/codesetcode	string	Lat15
console-setup	console-setup/charmap47	select	UTF-8
console-setup	console-setup/store_defaults_in_debconf_db	boolean	true
console-setup	console-setup/fontsize	string	16x28
EOF

  cat <<'EOF' | sudo tee /mnt/etc/preseed/locales.conf > /dev/null
...
EOF
  ...
  cat <<'EOF' | sudo tee /mnt/etc/default/console-setup > /dev/null
...
EOF
  ...
  cat <<'EOF' | sudo tee /mnt/etc/systemd/system/zfs-import-bpool.service > /dev/null
[Unit]
...
EOF
  ...
  sudo chroot \
    ...
    /bin/bash -c '
      ...
      zed -p /var/run/zed.pid
      sleep 2
      while ! [ -s /etc/zfs/zfs-list.cache/rpool ]; do
        zfs set canmount=noauto rpool/ROOT/debian
        sleep 2
      done
      while ! [ -s /etc/zfs/zfs-list.cache/bpool ]; do
  ...
  sudo sed -i "s/GRUB_CMDLINE_LINUX_DEFAULT=.*/GRUB_CMDLINE_LINUX_DEFAULT=\"${KERNEL_CMDLINE}\"/" /mnt/etc/default/grub
  sudo sed -i "s/GRUB_CMDLINE_LINUX=.*/GRUB_CMDLINE_LINUX=\"root=ZFS=rpool\/ROOT\/debian\"/" /mnt/etc/default/grub
  sudo sed -i "s/#GRUB_TERMINAL=.*/GRUB_TERMINAL=\"console\"/" /mnt/etc/default/grub
  sudo sed -i "s/#GRUB_DISABLE_LINUX_UUID=.*/GRUB_DISABLE_LINUX_UUID=true/" /mnt/etc/default/grub
  cat /mnt/etc/default/grub

  for i in "${!DISKS_PARTS_ESP[@]}"; do
    DISKPART="${DISKS_PARTS_ESP[$i]}"
    echo "Installing grub on ${DISKPART}"
    sudo mkdosfs -F 32 -s 1 -n EFI "${DISKPART}"
    sudo mkdir "/mnt/boot/efi-${i}"
    echo "${DISKPART} /boot/efi-${i} vfat defaults 0 0" | sudo tee -a /mnt/etc/fstab
    sudo mount "${DISKPART}" "/mnt/boot/efi-${i}"
    sudo chroot \
      /mnt \
      /usr/bin/env \
        i="${i}" \
