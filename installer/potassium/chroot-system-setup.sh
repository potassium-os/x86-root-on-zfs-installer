...
function _main () {
  ...
  _log "TODO: chroot into the new system and set it up"

  sudo -E mkdir /mnt/etc/zfs
  sudo -E cp /etc/zfs/zpool.cache /mnt/etc/zfs/

  TARGET_HOSTNAME=$(yq '.hostname' <<< "${INSTALLER_CONF_YAML}")
  export TARGET_HOSTNAME

  sudo -E hostname "${TARGET_HOSTNAME}"
  "$(hostname)" | sudo -E tee /mnt/etc/hostname

  cat <<EOF | sudo -E tee /mnt/etc/hosts
127.0.0.1 localhost
...
EOF
  ...
  NETWORK_INTERFACES=$(yq '.network_interfaces[]' <<< "${INSTALLER_CONF_YAML}")
  export NETWORK_INTERFACES

  SSH_PUBKEY=$(yq '.ssh_pubkey' <<< "${INSTALLER_CONF_YAML}")
  export SSH_PUBKEY

  KERNEL_CMDLINE=$(yq '.kernel_cmdline' <<< "${INSTALLER_CONF_YAML}")
  export KERNEL_CMDLINE

  ROOT_PASSWORD=$(yq '.root_password' <<< "${INSTALLER_CONF_YAML}")
  export ROOT_PASSWORD

  _log "${NETWORK_INTERFACES}"

  sudo -E mkdir -p /mnt/root/.ssh/authorized_keys
  cat <<EOF | sudo -E tee /mnt/root/.ssh/authorized_keys
${SSH_PUBKEY}
EOF

  cat <<EOF | sudo -E tee /mnt/etc/network/interfaces
auto lo
iface lo inet loopback
EOF

  cat <<EOF | sudo -E tee /mnt/etc/apt/sources.list.d/debian.sources
Types: deb
...
EOF
  ...
  sudo -E mount --make-private --rbind /dev  /mnt/dev
  sudo -E mount --make-private --rbind /proc /mnt/proc
  sudo -E mount --make-private --rbind /sys  /mnt/sys

  sudo -E mkdir -p /mnt/etc/preseed
  cat <<'EOF' | sudo -E tee /mnt/etc/preseed/console-setup.conf
...
EOF
  ...
  cat <<'EOF' | sudo -E tee /mnt/etc/preseed/locales.conf
...
EOF
  ...
  cat <<'EOF' | sudo -E tee /mnt/etc/default/console-setup
...
EOF
  ...
  cat <<'EOF' | sudo -E tee /mnt/etc/systemd/system/zfs-import-bpool.service
[Unit]
...
EOF
  ...
  cat <<'EOF' | sudo -E tee /mnt/chroot-setup.sh
#!/bin/bash

export DEBIAN_FRONTEND=noninteractive
apt-get -yq update
apt-get -yq install console-setup locales
...
for i in "${!DISKS_PARTS_ESP[@]}"; do
  mkdosfs -F 32 -s 1 -n EFI \${DISKS_PARTS_ESP[i]}
  mkdir /boot/efi${i}
  echo /dev/disk/by-uuid/$(blkid -s UUID -o value ${DISKS_PARTS_ESP[i]}) \
     /boot/efi${i} vfat defaults 0 0 >> /etc/fstab
  mount /boot/efi${i}
  grub-install --target=x86_64-efi --efi-directory=/boot/efi${i} \
    --bootloader-id=debian-${i} --recheck --no-floppy
done

apt-get -yq purge os-prober
...
EOF
  ...
  sudo -E chroot \
    ...
    /usr/bin/env \
      ...
      DISKS_PARTS_ESP="${DISKS_PARTS_ESP}" \
      SSH_PUBKEY="${SSH_PUBKEY}" \
      KERNEL_CMDLINE="${KERNEL_CMDLINE}" \
      ROOT_PASSWORD="${ROOT_PASSWORD}" \
    /bin/bash /chroot-setup.sh
}
